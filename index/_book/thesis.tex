...
% Everything below added by CII
  ...
  As you see with \texttt{\$2 \textbackslash{}pi\$} above, mathematics can
  be added by surrounding the mathematical text with dollar signs. More
  examples of this are in \hyperref[math-sci]{Mathematics and Science} if
  you uncomment the code in \hyperref[math]{Math}.
  ...
  \hyperdef{}{math}{\section{Math}\label{math}}
  ...
  data frame in \textbf{R} that you saw in \hyperref[rmd-basics]{R
  Markdown Basics} using the \texttt{kable} function, you can also create
  tables using \emph{pandoc}. (More information is available at
  ...
  specifying \texttt{"scale= "}. Here we use the mathematical graph stored
  in the ``subdivision.pdf'' file.
  ...
  hidden throughout the document (using the \texttt{include = FALSE} chunk
  tag) to help with readibility and/or setup.
